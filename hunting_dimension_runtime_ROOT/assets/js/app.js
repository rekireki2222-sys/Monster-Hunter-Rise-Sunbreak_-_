(() => {
  ...
  /* ===========================================================================
   * セーブデータ管理（バックアップ / 復元 / 堅牢化）  ※Phase 1
   * ---------------------------------------------------------------------------
   *  - 保存先は従来どおり localStorage（キー: STORAGE_KEY、構造: {inventory, meta}）。
   *    後方互換のためキー名・構造は変更しない。IndexedDB へは“二重書き”でミラーする。
   *  - セーブコード: 状態を versioned JSON にまとめ、UTF-8 安全な base64 で文字列化。
   *    外部ライブラリは追加しない（base64 は標準APIで完結＝依存ゼロ・ビルド不要）。
   *    コードが長くなり過ぎる場合は .json ダウンロードを使う方針（将来 lz-string 追加余地あり）。
   * ======================================================================== */

  const SAVE_SCHEMA_VERSION = 2;               // セーブコードのスキーマ版（localStorage v2 と整合）
  const SAVE_CODE_PREFIX = 'HD1:';             // セーブコードの識別子（誤貼り付け検出用）
  const BACKUP_KEY = STORAGE_KEY + '__backup'; // 初期化/上書き直前の自動退避先
  const IDB_NAME = 'hunting-dimension';
  const IDB_STORE = 'saves';
  const IDB_KEY = 'current';

  /* --- UTF-8 安全な base64（日本語の履歴も壊さない） --- */
  const bytesToBase64 = (bytes) => {
    let bin = '';
    const CHUNK = 0x8000; // 一度に渡し過ぎるとスタック溢れするため分割
  ...
  /* --- IndexedDB ミラー（localStorage が飛んでも復元候補にする） --- */
  const idbOpen = () => new Promise((resolve, reject) => {
    if (!('indexedDB' in window)) { reject(new Error('IndexedDB 非対応')); return; }
    const req = indexedDB.open(IDB_NAME, 1);
    req.onupgradeneeded = () => { try { req.result.createObjectStore(IDB_STORE); } catch (_) { /* noop */ } };
  ...
  /* --- localStorage が使えるか判定（プライベートモード/容量超過対策） --- */
  const storageAvailable = () => {
    try { const k = '__hd_test__'; localStorage.setItem(k, '1'); localStorage.removeItem(k); return true; }
    catch (_) { return false; }
  };

  /* --- 現在状態の保存（localStorage ＋ IndexedDB の二重書き） --- */
  const persistAll = ({ silent = false } = {}) => {
    const payload = JSON.stringify({ inventory: state.inventory, meta: state.meta });
    let ok = true;
    try {
      ...
      if (!silent) toast('⚠️ ブラウザ保存に失敗（容量超過/プライベートモード?）。セーブコードの控えを推奨します', 6000);
    }
    idbPut(payload); // 失敗してもよい（保険のミラー）
    return ok;
  };

  /* --- 状態 → versioned バンドル --- */
  const buildSaveBundle = () => ({
    app: 'hunting-dimension',
    version: SAVE_SCHEMA_VERSION,
    savedAt: new Date().toISOString(),
    data: { inventory: state.inventory, meta: state.meta }
  });

  /* --- セーブコード（base64文字列）化 --- */
  const encodeSaveCode = () => SAVE_CODE_PREFIX + utf8ToBase64(JSON.stringify(buildSaveBundle()));

  /* --- セーブコード/JSON を検証してデータ部を取り出す（不正なら throw） --- */
  const parseSaveInput = (raw) => {
    const text = String(raw == null ? '' : raw).trim();
    if (!text) throw new Error('内容が空です');
    let jsonText;
    if (text.startsWith(SAVE_CODE_PREFIX)) {
      jsonText = base64ToUtf8(text.slice(SAVE_CODE_PREFIX.length));
    } else if (text.startsWith('{')) {
      jsonText = text; // .json ファイルの生JSON
    } else {
      try { jsonText = base64ToUtf8(text); } catch (_) { throw new Error('コードの形式が不正です'); }
    }
    let bundle;
    try { bundle = JSON.parse(jsonText); } catch (_) { throw new Error('JSON として読み取れません'); }
    if (!bundle || typeof bundle !== 'object') throw new Error('データ構造が不正です');
    // {app,version,data:{inventory,meta}} と、後方互換の {inventory,meta} 直書きの両対応
    const data = (bundle.data && typeof bundle.data === 'object') ? bundle.data : bundle;
    if (!data || (!data.inventory && !data.meta)) throw new Error('セーブ内容が見つかりません');
    return { version: Number(bundle.version) || 1, data };
  };

  /* --- バンドルのデータ部を現在状態へ適用（＝復元）。保存＆再描画まで行う --- */
  const applySaveData = (data) => {
    state.inventory = normalizeInventory(data.inventory || {});
    const m = safeObject(data.meta);
    state.meta = {
      armorLevels: safeObject(m.armorLevels),
      coinHistory: safeArray(m.coinHistory),
      gachaHistory: safeArray(m.gachaHistory),
      miscHistory: safeArray(m.miscHistory),
      latestEvent: m.latestEvent ?? null,
      latestMonster: m.latestMonster ?? null,
  ...
  /* --- 「元に戻す」用バックアップへ退避 / 復元 --- */
  const stashBackup = () => {
    try { localStorage.setItem(BACKUP_KEY, JSON.stringify({ inventory: state.inventory, meta: state.meta, at: Date.now() })); }
    catch (_) { /* 退避失敗は致命でない */ }
  };
  const hasBackup = () => { try { return !!localStorage.getItem(BACKUP_KEY); } catch (_) { return false; } };
  const restoreBackup = () => {
    let raw = null;
    try { raw = localStorage.getItem(BACKUP_KEY); } catch (_) { /* noop */ }
    if (!raw) { alert('元に戻せるバックアップがありません。'); return; }
    try {
      const b = JSON.parse(raw);
      applySaveData({ inventory: b.inventory, meta: b.meta });
      try { localStorage.removeItem(BACKUP_KEY); } catch (_) { /* noop */ }
      renderBackupArea();
      toast('直前の状態に戻しました');
    } catch (_) { alert('バックアップの復元に失敗しました。'); }
  };

  /* --- 軽量トースト通知（ダークテーマに合わせた控えめな表示） --- */
  let toastTimer = null;
  ...
  /* --- 危険操作の確認モーダル（キーワード入力を要求しワンタップ事故を防ぐ） --- */
  const showDangerConfirm = ({ title, body, keyword, confirmLabel = '実行する' }) => new Promise((resolve) => {
    const backdrop = document.createElement('div');
    backdrop.className = 'selection-modal-backdrop';
    const modal = document.createElement('div');
    modal.className = 'selection-modal hd-danger-modal';
    modal.innerHTML = `
      <div class="selection-modal-header">
        <h3>${escapeHtml(title)}</h3>
        <p class="muted">${formatMultilineText(body)}</p>
      </div>
      <input type="text" class="hd-danger-input" autocomplete="off" placeholder="${escapeHtml(keyword)}">
      <div class="selection-modal-actions hd-danger-actions">
        <button type="button" class="ghost-button hd-cancel">キャンセル</button>
        <button type="button" class="danger-button hd-ok" disabled>${escapeHtml(confirmLabel)}</button>
      </div>
    `;
    backdrop.appendChild(modal);
    document.body.appendChild(backdrop);

    const input = modal.querySelector('.hd-danger-input');
    const okBtn = modal.querySelector('.hd-ok');
    const close = (val) => { document.removeEventListener('keydown', onKey); backdrop.remove(); resolve(val); };
    const onKey = (e) => { if (e.key === 'Escape') close(false); };
    document.addEventListener('keydown', onKey);
    backdrop.addEventListener('click', (e) => { if (e.target === backdrop) close(false); });
    input.addEventListener('input', () => { okBtn.disabled = input.value.trim() !== keyword; });
    modal.querySelector('.hd-cancel').addEventListener('click', () => close(false));
    okBtn.addEventListener('click', () => { if (input.value.trim() === keyword) close(true); });
    input.focus();
  });

  /* --- 通常の確認モーダル（インポートの上書き確認など） --- */
  const showConfirm = ({ title, body, confirmLabel = 'OK', danger = false }) => new Promise((resolve) => {
    const backdrop = document.createElement('div');
    backdrop.className = 'selection-modal-backdrop';
    const modal = document.createElement('div');
    ...
    modal.innerHTML = `
      ...
      <div class="selection-modal-actions hd-danger-actions">
        <button type="button" class="ghost-button hd-cancel">キャンセル</button>
        <button type="button" class="${danger ? 'danger-button' : 'primary-button'} hd-ok">${escapeHtml(confirmLabel)}</button>
      </div>
    `;
    backdrop.appendChild(modal);
    document.body.appendChild(backdrop);
    ...
    backdrop.addEventListener('click', (e) => { if (e.target === backdrop) close(false); });
    modal.querySelector('.hd-cancel').addEventListener('click', () => close(false));
    modal.querySelector('.hd-ok').addEventListener('click', () => close(true));
  });

  /* --- エクスポート（コピー / ファイルダウンロード） --- */
  const exportSaveToClipboard = async () => {
    const code = encodeSaveCode();
    const out = byId('saveCodeOutput');
    if (out) out.value = code;
    try { await navigator.clipboard.writeText(code); toast('セーブコードをコピーしました'); }
    catch (_) { if (out) { out.focus(); out.select(); } toast('自動コピー不可。表示中のコードを手動でコピーしてください', 5000); }
  };
  const exportSaveToFile = () => {
    const blob = new Blob([JSON.stringify(buildSaveBundle(), null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    const stamp = new Date().toISOString().replace(/[:T]/g, '-').slice(0, 19);
    a.href = url; a.download = `hunting-dimension-save_${stamp}.json`;
    document.body.appendChild(a); a.click(); a.remove();
    window.setTimeout(() => URL.revokeObjectURL(url), 1000);
    toast('セーブファイルをダウンロードしました');
  };

  /* --- インポート（コード / ファイル）。上書き前に確認＆自動バックアップ --- */
  const importSaveFromText = async (raw) => {
    let parsed;
    try { parsed = parseSaveInput(raw); }
    catch (error) { alert('読み込めませんでした: ' + error.message); return false; }

    const ok = await showConfirm({
      title: 'セーブデータを復元',
      body: '現在のデータは上書きされます。\n（念のため直前の状態は自動バックアップされ、「元に戻す」で戻せます）',
    ...
    stashBackup();               // 上書き前に退避
    applySaveData(parsed.data);  // 復元（保存＆再描画まで）
    renderBackupArea();
    toast('セーブデータを復元しました');
    return true;
  };
  const importSaveFromFile = (file) => {
    if (!file) return;
  ...
  /**
   * localStorage 保存（従来の「保存」ボタン）。二重書き＋トーストに刷新。
   */
  const saveLocal = () => {
    const ok = persistAll();
  ...
  /**
   * 初期化（誤操作対策つき）
   *  - キーワード入力式の確認モーダルを必須化（ワンタップでは絶対に消えない）
   *  - 実行直前に現在状態を自動バックアップ（「元に戻す」導線を表示）
   *  - 空状態を保存して固定（初期装備の再配布を防止＝まっさらを維持）
   */
  const resetLocal = async () => {
    const confirmed = await showDangerConfirm({
      title: 'データの初期化（全消去）',
      body: 'すべての所持品・強化・履歴・コインが消えます。\nこの操作は元に戻しにくいため、先に「セーブコード/ファイル」での控えを推奨します。\n\n実行するには下の欄に「初期化」と入力してください。',
      keyword: '初期化',
      confirmLabel: '初期化する'
    });
    if (!confirmed) return;

    stashBackup(); // 直前の状態を退避（「元に戻す」用）

    // 完全に空へ（テンプレ非依存で必ずまっさら）
    state.inventory = normalizeInventory({});
    state.meta = {
      armorLevels: {}, coinHistory: [], gachaHistory: [], miscHistory: [],
      latestEvent: null, latestMonster: null, loadErrors: state.meta.loadErrors || []
    };
    persistAll();        // 空状態を保存して固定（再読込で初期装備が復活しない）
    renderAll();
    renderBackupArea();  // 「元に戻す」ボタンを表示
    toast('初期化しました（「元に戻す」で直前の状態に復元できます）', 5000);
  };

  /* --- バックアップ/危険操作カードの動的部分（「元に戻す」ボタンの出し分け） --- */
  const renderBackupArea = () => {
    const undo = byId('undoResetArea');
    if (!undo) return;
    undo.innerHTML = hasBackup()
      ? '<button type="button" id="undoResetButton" class="ghost-button">↩ 直前の状態に戻す</button>'
      : '';
    const btn = byId('undoResetButton');
    if (btn) btn.addEventListener('click', restoreBackup);
  };

  /* ===========================================================================
   * クラウド保存（Cloudflare KV）  ※Phase 2
   * ---------------------------------------------------------------------------
   *  - Worker（別フォルダ cloudflare-worker/）をデプロイし、その URL を画面の
   *    「クラウドAPI URL」欄に入れると有効化（localStorage 保存。コード再編集不要）。
   *  - 保存: 初回は POST /api/save → {id,key} を発行。以降は PUT /api/save/:id（要 key）。
   *  - 復元: GET /api/save/:id（ID のみで読める）→ 既存の import 検証フローで取り込む。
   *  - セキュリティ: 読みは ID を知る人なら可能 / 書きは秘密 key 必須（詳細は worker の README）。
   * ======================================================================== */
  const CLOUD_API_DEFAULT = '';            // 既定URL（ここに直書きも可。通常は画面から設定）
  const CLOUD_API_LS = 'hd-cloud-api';     // APIベースURLの保存先(localStorage)
  const CLOUD_CRED_LS = 'hd-cloud-cred';   // 発行された {id,key} の保存先(localStorage)

  const getCloudApiBase = () => {
    let v = CLOUD_API_DEFAULT;
    try { v = localStorage.getItem(CLOUD_API_LS) || CLOUD_API_DEFAULT; } catch (_) { /* noop */ }
    return String(v || '').trim().replace(/\/+$/, '');
  };
  const setCloudApiBase = (url) => {
    try { localStorage.setItem(CLOUD_API_LS, String(url || '').trim()); } catch (_) { /* noop */ }
    renderCloudArea();
    toast('クラウドAPIのURLを保存しました');
  };
  const getCloudCred = () => { try { return JSON.parse(localStorage.getItem(CLOUD_CRED_LS)) || null; } catch (_) { return null; } };
  const setCloudCred = (c) => { try { localStorage.setItem(CLOUD_CRED_LS, JSON.stringify(c)); } catch (_) { /* noop */ } };

  /* クラウドへ保存（初回POST=発行 / 2回目以降PUT=更新） */
  const cloudSave = async () => {
    const base = getCloudApiBase();
    if (!base) { alert('先に「クラウドAPI URL」を設定してください。'); return; }
    const body = JSON.stringify(buildSaveBundle());
    const cred = getCloudCred();
    try {
      let res;
      if (cred && cred.id && cred.key) {
        res = await fetch(`${base}/api/save/${encodeURIComponent(cred.id)}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', 'X-Save-Key': cred.key },
          body
        });
        if (res.status === 404) { // サーバ側で失効していたら新規発行へフォールバック
          res = await fetch(`${base}/api/save`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body });
        }
      } else {
        res = await fetch(`${base}/api/save`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body });
      }
      ...
      if (data && data.id) setCloudCred({ id: data.id, key: data.key || (cred && cred.key) || '' });
      renderCloudArea();
      toast('クラウドに保存しました。復元IDを控えてください', 5000);
    } catch (err) {
      console.error('クラウド保存失敗:', err);
      alert('クラウド保存に失敗しました: ' + err.message + '\n（URL設定やネットワークをご確認ください）');
    }
  };

  /* クラウドから復元（IDで取得 → 既存の検証＋上書き確認＋自動バックアップで取り込む） */
  const cloudRestore = async (idRaw, keyRaw) => {
    const base = getCloudApiBase();
    if (!base) { alert('先に「クラウドAPI URL」を設定してください。'); return; }
    const id = String(idRaw || '').trim();
    if (!id) { alert('復元ID を入力してください。'); return; }
    try {
      const res = await fetch(`${base}/api/save/${encodeURIComponent(id)}`);
      if (res.status === 404) { alert('その ID のデータが見つかりません。'); return; }
      if (!res.ok) throw new Error('HTTP ' + res.status);
      const text = await res.text();
      const ok = await importSaveFromText(text); // 検証＋上書き確認＋自動バックアップを再利用
      if (ok) {
        const key = String(keyRaw || '').trim() || (getCloudCred() || {}).key || '';
        setCloudCred({ id, key });
        renderCloudArea();
      }
    } catch (err) {
      console.error('クラウド復元失敗:', err);
      alert('クラウド復元に失敗しました: ' + err.message);
    }
  };

  /* クラウドカードの状態表示（URL設定状況・現在の復元ID） */
  const renderCloudArea = () => {
    const area = byId('cloudStatusArea');
    if (!area) return;
    const base = getCloudApiBase();
    const cred = getCloudCred();
    const urlInput = byId('cloudApiInput');
    if (urlInput && document.activeElement !== urlInput) urlInput.value = base;
    if (!base) {
      area.innerHTML = '<span class="muted">クラウド未設定（下の「クラウドAPIの設定」に Worker の URL を入れてください）</span>';
    } else if (cred && cred.id) {
      area.innerHTML = `現在の復元ID: <code class="hd-inline-code">${escapeHtml(cred.id)}</code><br><span class="muted">別端末ではこの ID で復元できます。上書き保存には書込キーが必要です。</span>`;
    } else {
      area.innerHTML = '<span class="muted">API設定済み。「クラウドに保存」を押すと復元IDが発行されます。</span>';
    }
  };

  /* ===========================================================================
   * クイック操作パネル（プレイ中ループの最短化）  ※常時コイン表示＋FABパネル
   * ---------------------------------------------------------------------------
   *  「狩猟終了 → コイン加算 → 討伐対象抽選 → ガチャ」をスクロールせず1か所で回す。
   *  すべて既存関数（addCoins/spendCoins/drawMonster/runGachaWithEffect/drawXGacha）を
   *  そのまま再利用＝履歴・保存・演出は通常操作と完全に同じ（ロジック二重化なし）。
   * ======================================================================== */

  // 討伐報酬（rules.json 準拠）。★→獲得コイン。ここを変えれば加算額を調整できる。
  const MONSTER_COIN_REWARDS = {
    1: 240, 2: 360, 3: 480, 4: 720, 5: 960, 6: 1200,
    7: 1680, 8: 2160, 9: 2760, 10: 3600, 11: 4560, 12: 6000
  };

  const QP_OPEN_LS = 'hd-quickpanel-open'; // パネル開閉状態
  const QP_MON_LS = 'hd-quick-monster';    // {dimension, star} を記憶
  const QP_GACHA_LS = 'hd-quick-gacha';    // 'weapon'|'armor'|'decoration'

  /* --- 常時コイン表示：カウントアップ＋増減デルタのふわっと表示 ---
   * 値ソースは state.inventory.coins（renderMetrics から呼ぶ）。表示先は
   *  ・topbar内チップ #quickCoinDisplay（最上部で見える）
   *  ・固定ピル #quickCoinFixedValue（スクロールで topbar が隠れたら出る）
   * の両方を同じ値で更新する（＝どこにいてもコインが見え、値も一致）。 */
  let lastQuickCoin = null;
  const COIN_DISPLAY_IDS = ['quickCoinDisplay', 'quickCoinFixedValue'];
  const setCoinText = (text) => COIN_DISPLAY_IDS.forEach((id) => { const el = byId(id); if (el) el.textContent = text; });
  const floatCoinDelta = (diff) => {
    // いま見えている方（スクロール時は固定ピル、上部では topbar チップ）にデルタを出す
    const fixed = byId('quickCoinFixed');
    const host = (fixed && fixed.classList.contains('show')) ? fixed : byId('quickCoinChip');
    if (!host || !diff) return;
    const tag = document.createElement('span');
    tag.className = 'coin-delta ' + (diff > 0 ? 'plus' : 'minus');
    tag.textContent = (diff > 0 ? '+' : '') + diff;
    host.appendChild(tag);
    window.setTimeout(() => tag.remove(), 1100);
  };
  const updateQuickCoin = (value) => {
    if (!byId('quickCoinDisplay') && !byId('quickCoinFixedValue')) return;
    if (lastQuickCoin === null) { setCoinText(String(value)); lastQuickCoin = value; return; }
    const from = lastQuickCoin, to = value, diff = to - from;
    lastQuickCoin = to;
    if (diff === 0) { setCoinText(String(value)); return; }
    floatCoinDelta(diff);              // +N / -N をふわっと表示
    const dur = 480, start = performance.now();
    const step = (now) => {            // 数字をカウントアップ
      const t = Math.min(1, (now - start) / dur);
      const eased = 1 - Math.pow(1 - t, 3);
      setCoinText(String(Math.round(from + diff * eased)));
      if (t < 1) requestAnimationFrame(step); else setCoinText(String(to));
    };
    requestAnimationFrame(step);
  };
  // スクロールで topbar が隠れたら固定コインピルを表示（topbar は環境により sticky が
  // 効かないため、確実に「常時表示」にするためのフォールバック）。
  const updateCoinFixedVisibility = () => {
    const pill = byId('quickCoinFixed');
    if (!pill) return;
    const y = window.scrollY || document.documentElement.scrollTop || document.body.scrollTop || 0;
    pill.classList.toggle('show', y > 70);
  };

  /* --- パネル開閉（localStorage に記憶） --- */
  const setQuickPanelOpen = (open) => {
    const panel = byId('quickPanel'); const fab = byId('quickPanelFab');
    if (panel) panel.classList.toggle('open', open);
    if (fab) { fab.setAttribute('aria-expanded', String(open)); fab.classList.toggle('active', open); }
    try { localStorage.setItem(QP_OPEN_LS, open ? '1' : '0'); } catch (_) { /* noop */ }
  };
  const toggleQuickPanel = () => {
    const panel = byId('quickPanel');
    setQuickPanelOpen(!(panel && panel.classList.contains('open')));
  };

  /* --- a) コイン：★別報酬ワンタップ／任意額＋メモ／減算（既存関数を再利用） --- */
  const quickAddReward = (star) => {
    const amount = MONSTER_COIN_REWARDS[star] || 0;
    addCoins(amount, `★${star}討伐報酬`); // ← 既存関数。coinHistory に自動記録
    renderAll();
  };
  const quickCoinCustom = (isAdd) => {
    const amount = Number(byId('quickCoinAmount')?.value ?? 0);
    const note = byId('quickCoinNote')?.value?.trim() || (isAdd ? '加算' : '減算');
    if (!Number.isFinite(amount) || amount <= 0) { toast('金額を入力してください'); return; }
    if (isAdd) { addCoins(amount, note); }
    else if (!spendCoins(amount, note)) { toast('コインが不足しています'); return; }
    renderAll();
  };

  /* --- b) モンスター抽選：パネルの絞り込みを記憶し drawMonster を再利用 --- */
  const quickDrawMonster = () => {
    const dimension = byId('quickMonsterDimension')?.value ?? 'all';
    const star = byId('quickMonsterStar')?.value ?? 'all';
    try { localStorage.setItem(QP_MON_LS, JSON.stringify({ dimension, star })); } catch (_) { /* noop */ }
    const chosen = drawMonster(dimension, star); // ← 既存関数を再利用（履歴も記録）
    const out = byId('quickMonsterResult');
    if (out && chosen) {
      out.innerHTML = `<div class="qp-monster-card"><strong>${escapeHtml(chosen.name)}</strong><span>★${chosen.star} / 第${chosen.dimension}次元</span></div>`;
    }
  };

  /* --- c) ガチャ：種別切替。既存セクションの設定を尊重し runGachaWithEffect を再利用 --- */
  const setQuickGachaType = (type) => {
    try { localStorage.setItem(QP_GACHA_LS, type); } catch (_) { /* noop */ }
    document.querySelectorAll('.qp-gacha-type').forEach((b) => b.classList.toggle('active', b.dataset.type === type));
  };
  const getQuickGachaType = () => {
    try { return localStorage.getItem(QP_GACHA_LS) || 'weapon'; } catch (_) { return 'weapon'; }
  };
  const quickGachaDraw = (button) => {
    const type = getQuickGachaType();
    if (type === 'armor') {
      ...
      runGachaWithEffect('armorResultArea', button, () => drawArmorGacha(tier, count, mode));
    } else if (type === 'decoration') {
      const mode = byId('decorationGachaMode')?.value ?? 'ticket';
      const count = Number(byId('decorationDrawCount')?.value ?? 1);
      runGachaWithEffect('decorationResultArea', button, () => drawDecorationGacha(count, mode));
    } else {
      ...
      runGachaWithEffect('weaponResultArea', button, () => drawWeaponGacha(tier, count, mode));
    }
  };
  // 結果の詳細セクションへスクロール（種別に応じて）。閉じてから移動。
  const quickGachaJump = () => {
    const type = getQuickGachaType();
    const targetId = type === 'armor' ? 'armorGachaPanel' : (type === 'decoration' ? 'decorationGachaPanel' : 'weaponGachaPanel');
    ...
    window.requestAnimationFrame(() => byId(targetId)?.scrollIntoView({ behavior: 'smooth', block: 'start' }));
  };

  /* --- パネル内ログ（既存履歴を再利用表示） --- */
  const renderQuickLog = () => {
    const el = byId('quickLog');
    if (!el) return;
    const coin = (state.meta.coinHistory || [])[0];
    const gacha = (state.meta.gachaHistory || [])[0];
    const mon = state.meta.latestMonster;
    const rows = [];
    if (gacha) rows.push(`<li><span class="qp-tag">ガチャ</span>${escapeHtml(gacha)}</li>`);
    if (coin) rows.push(`<li><span class="qp-tag">コイン</span>${escapeHtml(coin)}</li>`);
    if (mon) rows.push(`<li><span class="qp-tag">討伐対象</span>${escapeHtml(mon)}</li>`);
    el.innerHTML = rows.length ? rows.join('') : '<li class="muted">まだ操作はありません</li>';
  };

  /* --- パネル初期化（保存済みの開閉・絞り込み・種別を復元） --- */
  const initQuickPanel = () => {
    let open = false;
    try { open = localStorage.getItem(QP_OPEN_LS) === '1'; } catch (_) { /* noop */ }
    setQuickPanelOpen(open);
    try {
      const m = JSON.parse(localStorage.getItem(QP_MON_LS) || 'null');
      if (m) {
        if (byId('quickMonsterDimension')) byId('quickMonsterDimension').value = m.dimension;
        if (byId('quickMonsterStar')) byId('quickMonsterStar').value = m.star;
      }
    } catch (_) { /* noop */ }
    setQuickGachaType(getQuickGachaType());
    renderQuickLog();
  };
  ...
  /**
   * 防具部位の「表示用」日本語ラベル。データ（armor.part = head/chest/...）は変更せず、
   * 画面表示だけ 頭/胴/腕/腰/足 に置き換える（照合・絞り込みロジックは英語キーのまま維持）。
   */
  const ARMOR_PART_LABELS = { head: '頭', chest: '胴', arms: '腕', waist: '腰', legs: '足' };
  const armorPartLabel = (part) => ARMOR_PART_LABELS[part] || part || '';
  ...
  const drawArmorGacha = (tier, drawCount = 1, paymentMode = 'ticket') => {
    ...
    for (let i = 0; i < drawCount; i += 1) {
      ...
      results.push({ ...chosen, __isNew: isNew, __method: methodLabel });
      pushLog('gachaHistory', `防具${tier}ガチャ[${paymentMode === 'ticket' ? 'イベント' : 'コイン'}]: ${chosen.name} [${armorPartLabel(chosen.part)}] (${chosen.rarity ?? '未設定'})`);
    }
  ...
  const applyEvent = async (event) => {
    ...
    const chooseArmorFromOwned = async () => {
      ...
      return await showSelectionModal({
        ...
        items: available,
        getTitle: (item) => item.name,
        getSubtitle: (item) => `${armorPartLabel(item.part)} / Lv+${clampArmorLevel(state.meta.armorLevels[item.id] ?? 0)}`
      });
    };
    ...
    const chooseArmorFromAll = async () => {
      ...
      return await showSelectionModal({
        ...
        items: available,
        getTitle: (item) => item.name,
        getSubtitle: (item) => `${armorPartLabel(item.part)} / ${item.rarity ?? '未設定'}`
      });
    };
   ...
   * モンスター抽選
   */
  // dimension/star を省略すると既存セレクトを読む（従来挙動）。クイックパネルからは
  // 引数で渡して呼ぶ。抽選結果（chosen）を返すので、パネル側でも結果表示に使える。
  const drawMonster = (dimensionArg, starArg) => {
    const dimension = (dimensionArg != null) ? dimensionArg : (byId('monsterDimensionFilter')?.value ?? 'all');
    const star = (starArg != null) ? starArg : (byId('monsterStarFilter')?.value ?? 'all');
    ...
    renderAll();
    return chosen; // パネル等から結果を受け取れるように
  };
  ...
  const renderArmorResults = (items) => {
    renderResultList('armorResultArea', items, (item) => `
      <div class="result-card gacha-card gacha-card--armor ${rarityEffectClass(item.rarity)}">
        ${acquireBadge(item)}
        <strong class="gacha-card-name">${escapeHtml(item.name)}</strong>
        <p class="gacha-card-rarity">${escapeHtml(item.rarity ?? '未設定')} / ${escapeHtml(armorPartLabel(item.part))}</p>
        <p class="gacha-card-method">入手方法: ${escapeHtml(item.__method ?? 'ガチャ')}</p>
      </div>
    `);
  };
  ...
  const renderArmorSelects = () => {
    ...
    select.innerHTML = uniqueOwned.length
      ? uniqueOwned.map((armor) => `<option value="${armor.id}">${armor.name} [${armorPartLabel(armor.part)}]</option>`).join('')
  ...
  const renderInventories = () => {
    ...
    const armorItems = [...new Set(state.inventory.armors)]
      ...
      .filter((entry) => entry.data && (!armorKeyword || `${entry.data.name} ${entry.data.part} ${armorPartLabel(entry.data.part)}`.toLowerCase().includes(armorKeyword)));

    if (armorList) {
      armorList.innerHTML = armorItems.length
        ? armorItems.map((entry) => `<li>${escapeHtml(entry.data.name)} [${escapeHtml(armorPartLabel(entry.data.part))}] Lv+${clampArmorLevel(state.meta.armorLevels[entry.id] ?? 0)}</li>`).join('')
        : '<li>なし</li>';
    }
  ...
  const renderMetrics = () => {
    ...
    updateQuickCoin(state.inventory.coins); // 常時コインヘッダー（同一ソース＋カウントアップ）
  };
  ...
  const renderArmorUpgradePreview = () => {
    ...
    const cost = Number(state.data.settings?.upgrade?.armor?.coin_cost ?? 20);
    result.innerHTML = `
      <p><strong>${escapeHtml(armor.name)}</strong> [${escapeHtml(armorPartLabel(armor.part))}]</p>
  ...
  const bindSearch = () => {
    ...
    input.addEventListener('input', () => {
      ...
      const merged = [
        ...
        ...state.data.armor.map((item) => ({ type: 'armor', name: `${item.name} [${armorPartLabel(item.part)}]`, text: item.rarity ?? '' })),
  ...
  const bindActions = () => {
    ...
    // セーブデータのバックアップ / 復元（Phase 1）
    byId('exportSaveCodeButton')?.addEventListener('click', exportSaveToClipboard);
    byId('downloadSaveButton')?.addEventListener('click', exportSaveToFile);
    byId('importSaveCodeButton')?.addEventListener('click', () => importSaveFromText(byId('importCodeInput')?.value));
    ...
    renderBackupArea(); // 「元に戻す」ボタンの初期表示

    // クラウド保存（Cloudflare KV）（Phase 2）
    byId('cloudSaveButton')?.addEventListener('click', cloudSave);
    byId('cloudRestoreButton')?.addEventListener('click', () => cloudRestore(byId('cloudRestoreId')?.value, byId('cloudRestoreKey')?.value));
    byId('cloudApiSaveButton')?.addEventListener('click', () => setCloudApiBase(byId('cloudApiInput')?.value));
    byId('copyCloudIdButton')?.addEventListener('click', async () => {
      const c = getCloudCred();
      if (!c || !c.id) { toast('まだ復元IDがありません。先に「クラウドに保存」を実行してください'); return; }
      try { await navigator.clipboard.writeText(c.id); toast('復元IDをコピーしました'); } catch (_) { toast('コピーできませんでした'); }
    });
    renderCloudArea(); // クラウド状態の初期表示

    // クイック操作パネル（既存関数を再利用して配線）
    byId('quickPanelFab')?.addEventListener('click', toggleQuickPanel);
    byId('quickPanelClose')?.addEventListener('click', () => setQuickPanelOpen(false));
    document.querySelectorAll('.qp-reward').forEach((b) => b.addEventListener('click', () => quickAddReward(Number(b.dataset.star))));
    byId('quickCoinAddButton')?.addEventListener('click', () => quickCoinCustom(true));
    byId('quickCoinSubButton')?.addEventListener('click', () => quickCoinCustom(false));
    byId('quickMonsterButton')?.addEventListener('click', quickDrawMonster);
    document.querySelectorAll('.qp-gacha-type').forEach((b) => b.addEventListener('click', () => setQuickGachaType(b.dataset.type)));
    byId('quickGachaButton')?.addEventListener('click', (event) => quickGachaDraw(event.currentTarget));
    byId('quickGachaJump')?.addEventListener('click', quickGachaJump);
    // topbar が隠れたら固定コインピルを出す（スクロール監視）
    window.addEventListener('scroll', updateCoinFixedVisibility, { passive: true });
    updateCoinFixedVisibility();
    initQuickPanel();
  ...
  const init = async () => {
    ...
    // 堅牢化：localStorage にセーブが無いが IndexedDB にミラーが残っている場合は復元提案。
    //   （ブラウザのデータ削除等で localStorage だけ消えたケースの救済。初回ユーザーは
    //    IDB も空なので何も起きない＝従来どおり初期装備の配布に進む。）
    let recovered = false;
    if (!saved && storageAvailable()) {
      const mirror = await idbGet();
      if (mirror) {
        try {
          const b = JSON.parse(mirror);
          if (b && (b.inventory || b.meta)
              && window.confirm('ブラウザのローカル保存は見つかりませんでしたが、バックアップ（IndexedDB）が見つかりました。復元しますか？')) {
            state.inventory = normalizeInventory(b.inventory || {});
            const m = safeObject(b.meta);
            state.meta = {
              armorLevels: safeObject(m.armorLevels),
              coinHistory: safeArray(m.coinHistory),
              gachaHistory: safeArray(m.gachaHistory),
            ...
            recovered = true;
            persistAll({ silent: true }); // localStorage へ戻して固定
          }
        } catch (_) { /* ミラー破損時は無視 */ }
      }
    }
    ...
    bindSearch();
    bindActions();
    // セーブも復元も無い「本当の初回」だけ初期装備を配布する
    if (!saved && !recovered) applyStartingLoadout();
    renderAll();
    ...
    window.HDApp = {
      ...
      resetLocal,
      // セーブ管理（Phase 1）
      exportSaveCode: encodeSaveCode,
      exportSaveToFile,
      importSaveFromText,
      restoreBackup,
      persistAll,
      // クラウド保存（Phase 2 / Cloudflare KV）
      cloudSave,
      cloudRestore,
      setCloudApiBase,
      drawWeaponGacha,
