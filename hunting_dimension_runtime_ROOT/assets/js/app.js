(() => {
  ...
  const resetLocal = () => {
    ...
    // 完全に空の状態を作る（inventory.json テンプレートに依存せず、必ず「まっさら」にする）。
    // coins=0 / 各チケット=0 / 武器・防具・装飾品・お守り=空 / 各履歴=空 になる。
    ...
    // ★重要：空の状態を localStorage に「保存」して固定する。
    //   以前は removeItem で消すだけだったため、次回読み込み時に saved=null となり
    //   init() の applyStartingLoadout() が走って初期装備（武器2・防具5）と
    //   ガチャ履歴が“復活”していた（＝初期化しても残る・ガチャ回数が0に戻らない原因）。
    //   空セーブを書き込めば saved が存在するので初期装備は再配布されず、まっさらが保たれる。
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ inventory: state.inventory, meta: state.meta }));
    } catch (error) {
      console.error('初期化の保存に失敗:', error);
    }
